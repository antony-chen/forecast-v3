"""Train and evaluate rolling-origin Direct168 MLP forecasting.

Protocol
--------
* Input load: the immediately preceding 168 hours.
* Known exogenous input: the complete following 168-hour forecast horizon.
* Output: the following 168 load values in one forward pass.
* Training origins move once per day at 00:00 by default (24-hour stride).
* Validation/test retain the original packed-week 00:00 origins.
* No load from the forecast horizon is supplied to the model.
"""
...
from Model_168hr_MLP_reverseweek_dailyrollingtrain import Direct168GlobalLocalModel, WEEK_HOURS  # noqa: E402
...
class Config:
    ...
    global_hidden: int = int(os.getenv("ONCOR_GLOBAL_HIDDEN", "64"))
    local_ext_hidden: int = int(os.getenv("ONCOR_LOCAL_EXT_HIDDEN", "24"))
    hourly_hidden: int = int(os.getenv("ONCOR_HOURLY_HIDDEN", "64"))
    local_correction_gain: float = float(os.getenv("ONCOR_LOCAL_CORRECTION_GAIN", "0.25"))
...
def build_model(cfg: Config, n_enc_ext: int, expert_specs, future_ext_dim: int, device):
    model = Direct168GlobalLocalModel(
        ...
        local_ext_hidden=cfg.local_ext_hidden,
        hourly_hidden=cfg.hourly_hidden,
        local_correction_gain=cfg.local_correction_gain,
    ).to(device)
...
def train(model, train_loader, val_loader, val_meta, cfg: Config, device, output: Path):
    ...
    print(
        f"[PROTOCOL] daily rolling-origin direct 168h; train stride={cfg.train_stride_hours}h; "
        "horizon-specific global head + per-hour local correction; validation/test use packed-week 00:00 origins"
    )
...
def run() -> None:
    ...
    config_payload = {
        **asdict(cfg),
        "method": "rolling-origin direct 168-hour global horizon head plus local hourly correction",
        ...
        "decoder_change": (
            "168 horizon-specific global base plus zero-initialized shared local correction; "
            "previous-week same-hour load is input only"
        ),
    }
