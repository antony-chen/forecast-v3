...
from Main_24hr_GRU_reverseweek import (
    Config,
    FORECAST_INDICES,
    _decode_from_z,
    _encode_once,
...
DEFAULT_RUN_NAME = os.getenv("ONCOR_RUN_NAME", "24hr_gru_reverseweek")
...
def plot_month(records: List[Dict], month: int, out_path: Path, profile_name: str, checkpoint_name: str, global_ylim: Tuple[float, float] | None = None):
    ...
    for i, rec in enumerate(records):
        ...
        title = (
            f"24hr GRU Reverse-Week | Profile {profile_name} | "
        ...
        if i == n - 1:
            ax.set_xlabel("Previous-week 168h history | forecast week assembled from seven 24h origins")

    fig.suptitle(
        f"24hr GRU Reverse-Week | Profile {profile_name} | {TARGET_YEAR}-{month:02d} | "
        "7 × 24h day-ahead forecasts stitched into one week | reverse encoder: full 168h, Sun→Mon"
