"""24-hour autoregressive GRU model used for the forward-only rolling-window ablation.
    ...
    def forward(
        ...
        if self.encoder_mode == "forward":
            ...
            if self.encoder_mode == "phase_aligned_dual":
                ...
                # Reverse the complete 168-hour week: Sunday 23:00 -> Monday 00:00.
...
# ============================================================
# Rolling observed-history GRU + autoregressive hourly GRU
# ============================================================

class VariationalDecoderHourlyGRU(nn.Module):
    """Original-style GRU horizon decoder used before Direct-24h MLP.

    The first GRU consumes the observed decoder history and supplies one hidden
    state per forecast origin. A second hourly GRU then generates the 24-hour
    horizon autoregressively. The first hourly input is the last observed load
    (GO token); subsequent inputs are the previous predicted means.
    """
    ...
    def __init__(
        ...
        # Rolling state over target-week observations available before each origin.
        self.rnn = nn.GRU(
            xprime_dim,
            latent_size,
            num_layers,
        ...
        # Dedicated autoregressive horizon decoder, matching the earlier GRU code.
    ...
    def _decode_autoregressive(
        self,
        hidden: torch.Tensor,
        go_value: torch.Tensor,
    ):
        if go_value.ndim == 1:
            go_value = go_value.unsqueeze(-1)
        if hidden.ndim != 2 or hidden.size(-1) != self.latent_size:
            raise ValueError(
                f"hidden must be [N,{self.latent_size}], got {tuple(hidden.shape)}"
            )
        if go_value.shape != (hidden.size(0), self.output_dim):
            raise ValueError(
                f"go_value must be [N,{self.output_dim}], got {tuple(go_value.shape)}"
            )

        h_hourly = hidden.unsqueeze(0)                 # [1,N,H]
        input_k = go_value.unsqueeze(1)                # [N,1,D]
        mu_list, logvar_list = [], []

        for _ in range(self.output_len):
            out_k, h_hourly = self.hourly_gru(input_k, h_hourly)
            state_k = out_k[:, 0, :]
            mu_k = self.head_mu(state_k)
            raw_lv_k = self.head_logvar(state_k)
            logvar_k = self.logvar_min + (
                self.logvar_max - self.logvar_min
            ) * torch.sigmoid(raw_lv_k)
            mu_list.append(mu_k)
    ...
    def forward(
        ...
        batch, steps, _ = x_l_seq.shape
        h_init = torch.stack(
            [self.project[i](z_latent) for i in range(self.num_layers)], dim=0
        )
        h0 = h_init[-1]
        ...
        mu_flat, lv_flat = self._decode_autoregressive(
...
class VariationalSeq2SeqMeta(nn.Module):
    def __init__(
        ...
        self.decoder = VariationalDecoderHourlyGRU(
            xprime_dim=xprime_dim,
