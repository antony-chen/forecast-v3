...
def build_predictions_table(records, profile_name: str) -> pd.DataFrame:
    ...
    Shared by every Plot_*_reverseweek.py script. Each record must provide
...
def display_png_in_notebook(path) -> None:
    ...
    try:
        ...
        # Matches run directory names such as "24hr_gru_reverseweek" or "168hr_mlp_reverseweek",
# as produced by ONCOR_RUN_NAME defaults in the Main_*_reverseweek.py scripts.
_RUN_NAME_RE = re.compile(r"^(?P<horizon>\d+hr)_(?P<model>[a-zA-Z0-9]+)_reverseweek$")
...
def _parse_run_name(run_name: str) -> Tuple[str, str]:
    ...
    if not m:
        raise ValueError(
            ...
            "expected a pattern like '24hr_gru_reverseweek'."
        )
    return m.group("horizon"), m.group("model").upper()


def discover_run_dirs(output_root) -> List[Path]:
    """Find run directories directly under an ONCOR_OUTPUT_ROOT that match the
    '<horizon>_<model>_reverseweek' naming convention (e.g. '24hr_gru_reverseweek')."""
...
def load_run_predictions(run_dir, checkpoint_name: str = "mse", year: Optional[int] = None) -> pd.DataFrame:
    ...
    Plot_*_reverseweek.py scripts) for a single run directory."""
...
def best_model_by_device(
    ...
    run_dirs: run directories such as '.../24hr_gru_reverseweek', '.../24hr_mlp_reverseweek',
    '.../168hr_gru_reverseweek', '.../168hr_mlp_reverseweek' — the horizon and model name
...
def _oncor_load_weekly_utils(df = [], load_col="Amps1A"):
    '''
    ...
    present_features = [load_col] + [f for f in expected_features if f in df.columns]
    ...
    for xfmr, g in df.groupby(group_col):
        ...
        print(
            f"FEEDER={xfmr}: weeks={tensor.shape[0]} (out of {possible_weeks}), "
...
def get_data_oncor_load_weekly(load_col="Amps1A", XFMR="all"):
    """Load weekly ONCOR data and construct calendar features correctly.

    Returns
    -------
    time_index : ndarray, shape [n_weeks, 168]
    load       : ndarray, KWH
    temp       : ndarray, selected weather-temperature channel
    workday    : ndarray, 1=weekday and 0=weekend
    season     : ndarray, 0=winter, 1=spring, 2=summer, 3=autumn

    Notes
    -----
    ``ONCOR_TEMP_FEATURE`` can select the temperature channel.  The default
    remains ``SURDPOINTTEMPFAHRENHEIT`` to preserve the current experiments.
    Calendar variables are always rebuilt from timestamps; they are never
    mapped from humidity or heat-index columns.
    """
    # Prefer an explicit project root. Otherwise search upward so this package
    # also works when it is placed inside a wrapper folder under M2oE2_For_Zhe.
    configured_root = os.environ.get("ONCOR_PROJECT_ROOT", "").strip()
    if configured_root:
        ...
        project_root = next(
            (p for p in candidates if (p / "processed_data").is_dir()),
            here.parent.parent,
        )

    if XFMR == "all":
        fpath = project_root / "processed_data" / "weeks_tensor_all.npz"
    else:
        fpath = project_root / "processed_data" / f"weeks_tensor_{XFMR}.npz"

    if not fpath.exists():
        raise FileNotFoundError(
            f"Original ONCOR weekly NPZ not found: {fpath}. "
            "Set ONCOR_PROJECT_ROOT to the M2oE2_For_Zhe directory."
        )

    print(f"[ONCOR_DATA] project_root={project_root}")
    print(f"[ONCOR_DATA] source={fpath}")
    data = np.load(fpath, allow_pickle=True)
    X = data["tensor"]
    features = data["features"].tolist()
    time_index = data["time_index"]

    if X.ndim != 3 or np.asarray(time_index).shape != X.shape[:2]:
        raise ValueError(
            f"Unexpected ONCOR shapes: tensor={X.shape}, "
            f"time_index={np.asarray(time_index).shape}"
        )

    temp_feature = os.environ.get(
        "ONCOR_TEMP_FEATURE", "SURDPOINTTEMPFAHRENHEIT"
    ).strip()
    required = [load_col, temp_feature]
    missing = [name for name in required if name not in features]
    if missing:
        raise KeyError(
            f"Missing required ONCOR features {missing}. "
            f"Available features include: {features}"
        )

    load = X[:, :, features.index(load_col)]
    temp = X[:, :, features.index(temp_feature)]

    # Rebuild calendar semantics from the actual hourly timestamps.
    dt = pd.DatetimeIndex(
        pd.to_datetime(np.asarray(time_index).reshape(-1), errors="raise")
    )
    workday = (dt.dayofweek < 5).astype(np.float32).reshape(time_index.shape)

    month = dt.month.to_numpy()
    season_flat = np.select(
        [
    ...
    season = season_flat.reshape(time_index.shape)

    print(
        f"[ONCOR_DATA] temp_feature={temp_feature}; "
        "workday=1 weekday/0 weekend; season=0 winter/1 spring/2 summer/3 autumn"
    )
