...
from Main_24hr_GRU_forwardonly_rollingtrain import (
    Config,
    FORECAST_INDICES,
    _decode_from_z,
    _encode_once,
...
DEFAULT_OUTPUT_ROOT = Path(os.getenv("ONCOR_OUTPUT_ROOT", "/content/drive/MyDrive/M2oE2_For_Zhe/oncor_reverseweek"))
DEFAULT_RUN_NAME = os.getenv("ONCOR_RUN_NAME", "24hr_gru_forwardonly_rollingtrain")
...
def build_cfg_from_run(run_dir: Path) -> Config:
    ...
    cfg = Config(
        encoder_mode=cfg_json["encoder_mode"],
        run_name=cfg_json["run_name"],
...
@torch.no_grad()
def predict_week(model, cfg: Config, enc_l, enc_ext, dec_l, dec_ext):
    selected, mu_z, logvar_z, phase_features = _encode_once(model, enc_l, enc_ext, cfg)
    mu, logvar = _decode_from_z(
        model,
        enc_l,
        dec_l,
        dec_ext,
        mu_z,  # deterministic z = mu
        phase_features,
        selected,
        cfg,
    )
...
def stitch_day_ahead(mu_raw: np.ndarray, sigma_raw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    # FORECAST_INDICES = [0,24,...,144], each row predicts the corresponding 24h block.
    week_mu = np.zeros(168, dtype=float)
    week_sigma = np.zeros(168, dtype=float)
    for j, origin in enumerate(FORECAST_INDICES):
        week_mu[origin : origin + 24] = mu_raw[j]
        week_sigma[origin : origin + 24] = sigma_raw[j]
    return week_mu, week_sigma
...
def build_week_records(model, cfg: Config, profiles: List[Dict], prepared: Dict, device: torch.device, target_profile: str):
    ...
    for week in range(n_weeks - 1):
        ...
        enc_ext = torch.tensor(enc_proc[ext_keys].to_numpy()[None, ...], dtype=torch.float32, device=device)
        dec_l = torch.tensor(dec_proc[["load"]].to_numpy()[:144][None, ...], dtype=torch.float32, device=device)
        dec_ext = torch.tensor(dec_proc[ext_keys].to_numpy()[:144][None, ...], dtype=torch.float32, device=device)

        pred_mu_norm, pred_logvar_norm = predict_week(model, cfg, enc_l, enc_ext, dec_l, dec_ext)
        pred_mu_raw = pred_mu_norm * scale + shift
        pred_sigma_raw = np.exp(0.5 * pred_logvar_norm) * abs(scale)
        week_mu_raw, week_sigma_raw = stitch_day_ahead(pred_mu_raw, pred_sigma_raw)

        true_week = dec_raw["load"].to_numpy(dtype=float)
        err = week_mu_raw - true_week
        rmse = float(np.sqrt(np.mean(err ** 2)))
        mae = float(np.mean(np.abs(err)))
        coverage = float(np.mean((true_week >= week_mu_raw - week_sigma_raw) & (true_week <= week_mu_raw + week_sigma_raw)))
        ...
        records.append(
            {
                ...
                "forecast_mu": week_mu_raw,
                "forecast_sigma": week_sigma_raw,
                "rmse": rmse,
                "mae": mae,
                "coverage_1sigma": coverage,
                "full_time": pd.date_range(enc_raw.index[0], periods=336, freq="h"),
            }
        )
    return records
...
def plot_month(records: List[Dict], month: int, out_path: Path, profile_name: str, checkpoint_name: str, global_ylim: Tuple[float, float] | None = None):
    ...
    for i, rec in enumerate(records):
        ax = axes[i]
        x_hist = np.arange(168)
        x_fc = np.arange(168, 336)

        full_time = pd.DatetimeIndex(rec["full_time"])
        day_ticks = np.arange(0, 336, 24)
        ...
        ax.axvline(167.5, color="0.4", ls="--", lw=1.0)
        ...
        title = (
            f"24hr GRU Forward-Only Rolling-Train | Profile {profile_name} | "
        ...
        if i == n - 1:
            ax.set_xlabel("Previous 168h history | forecast week assembled from seven daily 00:00 24h origins")

    fig.suptitle(
        f"24hr GRU Forward-Only Rolling-Train | Profile {profile_name} | {TARGET_YEAR}-{month:02d} | "
        "7 × 24h day-ahead forecasts stitched into one week | forward encoder: chronological 168h history, hourly-rolling training"
...
def main():
    ...
    model = build_model(cfg, len(prepared["ext_keys"]), expert_specs, device)
